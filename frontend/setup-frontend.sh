...
export const videoAPI = {
  searchVideos: (query, maxResults = 12) => 
    api.get('/videos/search', { params: { query, maxResults } }),
  getLatestVideos: (maxResults = 12) =>
    api.get('/videos/latest', { params: { maxResults } }),
  getVideoDetails: (videoId) => 
    api.get(\`/videos/details/\${videoId}\`),
  requestVideo: (videoUrl) => 
    api.post('/videos/request', { videoUrl }),
};

export const channelAPI = {
  getApprovedChannels: () => api.get('/channels'),
  addApprovedChannel: (channelId, channelName) => 
    api.post('/channels', { channelId, channelName }),
  removeApprovedChannel: (id) => api.delete(\`/channels/\${id}\`),
  searchChannels: (query) => 
    api.get('/channels/search', { params: { query } }),
};

export const keywordAPI = {
  getBlockedKeywords: () => api.get('/keywords'),
  addBlockedKeyword: (keyword) => 
    api.post('/keywords', { keyword }),
  removeBlockedKeyword: (id) => api.delete(\`/keywords/\${id}\`),
  bulkAddKeywords: (keywords) => 
    api.post('/keywords/bulk', { keywords }),
};
